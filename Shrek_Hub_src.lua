...
Stand:addToggle("Toggle Stand Farm", nil, function(looptoggle)
    print(looptoggle)
while looptoggle == true do
    if game.Players.LocalPlayer.Character:WaitForChild("StandName").Value ~= standgetvalue then
repeat
workspace.Pucci.Pucci:FireServer()
        game:GetService("ReplicatedStorage").ItemEvent.StandArrow:FireServer()
        print(game.Players.LocalPlayer.Character:WaitForChild("StandName").Value)
        wait(4)
until game.Players.LocalPlayer.Character:WaitForChild("StandName").Value == standgetvalue or looptoggle == false
wait()
break;
end
wait()
